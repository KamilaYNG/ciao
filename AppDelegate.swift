...
class AppDelegate: UIResponder, UIApplicationDelegate, CoreDataDelegate {
    ...
    func loadPlacement(withID placementID: String?) throws {
        ...
        var sdk:VungleSDK = VungleSDK.shared()
        do {
            try sdk?.loadPlacement(withID: <INTERST-4932529>)
    ...
    func playAd(_ controller: UIViewController?, options: [AnyHashable : Any]?, placementID: String?) throws {
        var sdk:VungleSDK = VungleSDK.shared()
        do {
            try sdk.playAd(self, options: nil, placementID: "INTERST-4932529")
        } catch let error as NSError {
    ...
    lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator? = {
        ...
        do {
            ...
            if (destinationModel?.isConfiguration(withName: "Default",
                ...
                let appID = "632827d6d62701ea78ac440d";
                var placementIDsArray:Array<String> = ["<INTERST-4932529>"];
                var sdk:VungleSDK = VungleSDK.shared()
                do {
                    try sdk.start(withAppId: "632827d6d62701ea78ac440d");
                } catch let error as NSError {
